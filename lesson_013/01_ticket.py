...
# TODO Файл создаётся на проект, но в силу структуры наших занятий - у нас каждый модуль - локальный проект
# TODO Поэтому и файл должен быть внутри папки lesson_013 с перечнем библиотек, которые используются в этом проекте
# TODO А это по сути, только PIL :)
def make_ticket(fio, from_, to, date):
    ...
    # TODO Я такой не пользовался, если честно, возможно это новинка :) Вроде как перечень этих хоткеев можно найти в
    # TODO Help/Keymap Reference. С чем может быть связано - не подскажу, надо гуглить
    # TODO Попробуйте покопать в сторону plugins - дополнительных модов, которые отдельно устаналиваются в PyCharm
