...
# как я усвоил, возможно ошибочно, файл создается на проект(окружение), поэтому он снаружи.
...
#  Файл создаётся на проект, но в силу структуры наших занятий - у нас каждый модуль - локальный проект
#  Поэтому и файл должен быть внутри папки lesson_013 с перечнем библиотек, которые используются в этом проекте
#  А это по сути, только PIL :)

# TODO: Создал внутри урока. Но зависимостей все равно много. Из лекции я понял, что для чистого файла requirements.txt
#  нужно создавать виртуальное окружение.
...
def make_ticket(fio, from_, to, date):
    ...
    #  это быстрая комбинация для добавления импорта, у меня не работает. Например я забыл
    ...
    #  Я такой не пользовался, если честно, возможно это новинка :) Вроде как перечень этих хоткеев можно найти в
    #  Help/Keymap Reference. С чем может быть связано - не подскажу, надо гуглить
    #  Попробуйте покопать в сторону plugins - дополнительных модов, которые отдельно устаналиваются в PyCharm
