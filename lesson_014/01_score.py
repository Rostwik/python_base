...
# TODO: Подскажите, пожалуйста, патерн "Состояние"
# TODO: Вы предлагаете применить к модулю подсчета очков или к модулю консольной части.
#  Я почитал ссылки, что указаны в описании, смысл понял, но не вижу как это применить к данной задаче.
# TODO Его применить можно к модулю подсчёта очков - для реализации первого и второго броска отдельно
# TODO Создаётся класс бросок - добавляются тонкости расчётов (как считывается символ 'X', как '/')
# TODO Затем в наследниках описываются первый бросок и второй (у первого не может быть spare '/',
# TODO У второго не может быть strike 'X')
# TODO И в итоге от броска (состояния - первый/второй) зависит трактовка символов
...
#         TODO Тут мы создаем объект класса, описанного в handling_external_data.py
#         TODO Сам класс реализует обработку данных полученных запросом с какого-нибудь сайта по заданному URL
#         TODO Запрос производится при помощи библиотеки requests и метода .get
#         fake_result = Mock()  TODO Тут мы создаём пустышку (подробнее тут https://habr.com/ru/post/141209/)
#         fake_result.text = _test_data
#         fake_get_result = Mock(return_value=fake_result)  TODO А здесь говорим этой пустышке возвращать данные
#         TODO При обращении к ней. Так, обращение к fake_get_result вернёт нам fake_result
#         TODO А точнее атрибут .text, который содержит строки заданные выше в _test_data
#         handling_external_data.requests.get = fake_get_result
#         TODO Здесь мы подменяем импортированный метод get, который должен обращаться к сайту и получать данные с него
#         TODO Другим элементом-пустышкой, который всегда будет возращать заданные выше строки
#         result = getter.run()
#         TODO Теперь мы можем протестировать наш класс, запустив его главный метод run
#         TODO Который в процессе будет получать данные при помощи requests.get, но тк мы подменили этот метод
#         TODO То данные придут не с сайта, а тестовые, которые мы создали сами
#         self.assertEqual(result, 9)
#         TODO А тут мы сравниваем результат обработки наших данных с тем, что должно получиться
# TODO Для понимания важно понять последовательность шагов
# TODO import в самом начале выполняет код из библиотеки requests
# TODO значит где-то в нашем поле имен создается метод get который обращается к внешнему ресурсу
# TODO Затем создаётся класс, который использует этот метод
# TODO Затем в тестовом модуле переменная get переопределяется
# TODO И далее, при обращении к ней будет уже не оригинальный метод выполняться, а тот, который мы указали
